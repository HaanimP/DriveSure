    ...
    <div class="profile-header">
      ...
      <div class="profile-content">
        ...
        <div class="profile-info">
          ...
          <p class="role-badge">{{ getMemberSince() }}</p>
        </div>
      </div>
    </div>
      ...
      <div class="info-section settings-section">
        ...
        <div class="setting-item">
          <label class="checkbox-label">
            <input type="checkbox" v-model="settings.requestUpdates">
            <span>Notify me when my request status updates</span>
          </label>
        </div>
...
const checkForRequestUpdates = async (userId) => {
  try {
    // Check if user has seen notifications today
    const lastCheck = localStorage.getItem('lastNotificationCheck');
    const today = new Date().toDateString();
    
    if (lastCheck === today) {
      return; // Already shown notifications today
    }
    ...
    const userRequests = response.data.filter(req => req.user_id === userId || req.customer_id === userId);

    // Check for requests with status changes in the last 24 hours (not pending)
    const updates = userRequests.filter(req => {
      const createdDate = new Date(req.created_at);
      const dayAgo = new Date(Date.now() - 24 * 60 * 60 * 1000);
      return createdDate > dayAgo && req.status !== 'pending';
    });

    if (updates.length > 0 && settings.value.requestUpdates) {
      requestUpdates.value = updates;
...
.background-gradient {
  ...
  right: 0;
  bottom: 0;
  background: linear-gradient(135deg, rgba(5, 20, 65, 0.8) 0%, rgba(0, 168, 232, 0.4) 100%);
  z-index: 0;
}
...
body.dark-mode .request-card .request-text {
  color: #333;
}

body.dark-mode .review-card .review-name,
body.dark-mode .review-card .review-plan {
  color: #333;
}
